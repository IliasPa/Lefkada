...
function prune() {
  ...
  // referendum must stay editable in /admin, however old) — and NEVER one that
  // has recorded a vote: the admin results view reads the definition from this
  // table, and results/participants must always have their referendum.
  del(
    'referendums',
    `select id::text as id from public.referendums
     where published and ends_at < now() - interval '${PRUNE_DAYS} days'
       and ('ref_' || id::text) not in (select distinct poll_id from public.referendums_participants)`,
